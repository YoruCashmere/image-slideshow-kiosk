	...
	:wxFrame(nullptr,wxID_ANY,title,wxDefaultPosition,wxDefaultSize, wxFRAME_NO_TASKBAR|wxBORDER_NONE)
{
	...
	Move(sz.x - (winSz.x-40), sz.y - (winSz.y + 60));
	...
	if (!data) {
		...
		if (!Imagepaths.empty()) {
			...
			if (count < 3) {
				...
				textureID = 0;
				glCanvas->Refresh();
				wxMessageBox("Some file in this folder are corrupted", "Curropted Files.");
			}				
		}
		return;
	}
...
std::string mainFrame::getImagePath()
{
	...
	*/
	std::string configPath = wxStandardPaths::Get().GetUserConfigDir().ToStdString() + "/App";
...
void mainFrame::swapp(wxMouseEvent& evt)
{
	...
	uploadImage(Imagepaths[currentIndex+1]);
